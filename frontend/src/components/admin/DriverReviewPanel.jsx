...
const DriverReviewPanel = ({ driver, onClose, onAction }) => {
  ...
  const endorsementCount = Array.isArray(driver.endorsements) ? driver.endorsements.length : 0;
  const equipmentCount = Array.isArray(driver.equipment_types) ? driver.equipment_types.length : 0;
  ...
  return <div className="fixed inset-0 z-40 bg-[#183327]/35" onMouseDown={onClose}>
    <aside className="ml-auto h-full w-full max-w-xl overflow-y-auto bg-[#fffdf8] shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-6 p-6">
        ...
        <section className="rounded-2xl border border-[#e6dfcb] bg-white p-5"><h3 className="font-bold text-[#29483a]">Profile details</h3><div className="mt-4 grid grid-cols-2 gap-5"><Field label="Phone" value={driver.phone} /><Field label="CDL class" value={driver.cdl_class} /><Field label="Experience" value={`${driver.years_experience ?? 0} years`} /><Field label="Availability" value={driver.availability} /><Field label="Preferred region" value={driver.preferred_region ? "Selected" : "No preference"} /><Field label="Endorsements" value={endorsementCount ? `${endorsementCount} selected` : "None selected"} /><Field label="Equipment" value={equipmentCount ? `${equipmentCount} selected` : "None selected"} /></div></section>
